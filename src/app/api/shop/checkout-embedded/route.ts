...
/** POST: 埋め込み用 Checkout セッション作成。body: { packId }。戻り値: { clientSecret }。Webhook は従来どおり checkout.session.completed */
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      return_url: `${baseUrl}/shop/return?session_id={CHECKOUT_SESSION_ID}`,
